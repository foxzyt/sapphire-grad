...
import "../../../../infinitum/versions/v1.0.0/files/main.sp"
...
class RNNCell {
    function init(double input_dim, double hidden_dim, string init_type) void {
        ...
        this.w_ih = Linear();
        this.w_ih.init(input_dim, hidden_dim, init_type);
        
        this.w_hh = Linear();
        this.w_hh.init(hidden_dim, hidden_dim, init_type);
    }
    
    function forward(list x, list h_prev, list h_next) void {
        var ih_out = listCreate();
        this.w_ih.forward(x, ih_out);
        ...
        var i = 0;
        while (i < this.hidden_dim) {
            var ih_t = listGet(ih_out, i);
            var hh_t = listGet(hh_out, i);
            
            var sum_t = Tensor();
            ih_t.add(hh_t, sum_t);
            
            var final_t = Tensor();
            sum_t.tanh_t(final_t);
            
            listAppend(h_next, final_t);
            i = i + 1;
        }
    }
...
// Nova função usando infinitum para normalização de gradientes
function normalize_gradients_infinitum(list gradients, list out_normalized) void {
    var grad_values = [];
    var i = 0;
    var len = listLength(gradients);
    while (i < len) {
        var t = listGet(gradients, i);
        grad_values[len(grad_values)] = t.grad;
        i = i + 1;
    }
    
    var grad_norm = sqrt(sum(mul(grad_values, grad_values)));
    
    if (grad_norm > 0.0000001) {
        var normalized = mul_scalar(grad_values, 1.0 / grad_norm);
        i = 0;
        while (i < len) {
            var t = listGet(gradients, i);
            t.grad = normalized[i];
            listAppend(out_normalized, t);
            i = i + 1;
        }
    } else {
        i = 0;
        while (i < len) {
            listAppend(out_normalized, listGet(gradients, i));
            i = i + 1;
        }
    }
}
